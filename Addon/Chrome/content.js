...
if (typeof chrome === 'undefined' || !chrome.storage) {
  try {
    ...
    if (hostname !== 'localhost' && hostname !== '127.0.0.1') {
      ...
      let lastPageActiveElement = null;

      document.addEventListener('focusin', (e) => {
        const target = e.target;
        if (target && target.id !== 'morgi-main-host' && target.id !== 'morgi-picker-host') {
          lastPageActiveElement = target;
        }
      }, true);

      // Overriding Event.prototype.target
      const originalTargetGetter = Object.getOwnPropertyDescriptor(Event.prototype, 'target')?.get;
      ...
      const originalSrcElementGetter = Object.getOwnPropertyDescriptor(Event.prototype, 'srcElement')?.get;
      ...
      const originalActiveElementGetter = Object.getOwnPropertyDescriptor(Document.prototype, 'activeElement')?.get;
      ...
      // Overriding FocusEvent.prototype.relatedTarget
      const originalRelatedTargetGetter = Object.getOwnPropertyDescriptor(FocusEvent.prototype, 'relatedTarget')?.get;
      if (originalRelatedTargetGetter) {
  ...
  // getServerUrl() removed — all API calls go through background.js (CORS/PNA bypass)
  ...
  async function showCategoryModal(imgUrl, imgElement) {
    ...
    // Pass imgElement into the modal logic
  ...
  function extractSourceUrl(imgElement) {
    ...
    // Walk up from imgElement to find the nearest anchor
